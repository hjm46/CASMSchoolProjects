 ...
 * one.
 * return the address of the beginning of the heap
...
void add_free(Block* free_block)
{
  ...
  free_list_head->freeNode.prevFree = free_block;
  free_block->freeNode.nextFree = free_list_head;
  free_list_head = free_block;
  free_list_head->freeNode.prevFree = NULL;
}

void remove_free(Block* free_block)
{
  Block* prev = free_block->freeNode.prevFree;
  Block* next = free_block->freeNode.nextFree;
...
void* mm_malloc(size_t size) {
  ...
  //if there's no existing size that fits the request
  if(ptrFreeBlock == NULL)
  {
    ptrFreeBlock = requestMoreSpace(sizeof(BlockInfo) + reqSize);
    if(ptrFreeBlock == NULL)
      return NULL;
  ...
  //otherwise ptrFreeBlock already exists
  if (ptrFreeBlock->info.size < (-reqSize -(long)sizeof(Block)))
  {
    remove_free(ptrFreeBlock);
    Block* next = next_block(ptrFreeBlock);
...
void coalesce(Block* blockInfo) {
  ...
  remove_free(blockInfo);
  while(previousBlock != NULL && previousBlock->info.size < 0)
  {
    remove_free(previousBlock);
    if(nextBlock!=NULL)
      nextBlock->info.prev = previousBlock;
    previousBlock->freeNode.nextFree = blockInfo->freeNode.nextFree;
    previousBlock->info.size = previousBlock->info.size + blockInfo->info.size - (long)sizeof(BlockInfo);
    blockInfo = previousBlock;
    previousBlock = previousBlock->info.prev;
  }

  while(nextBlock != NULL && nextBlock->info.size < 0)
  {
    remove_free(nextBlock);
    blockInfo->freeNode.nextFree = nextBlock->freeNode.nextFree;
    blockInfo->info.size = blockInfo->info.size + nextBlock->info.size - (long)sizeof(BlockInfo);
...
void mm_free(void* ptr) {
  ...
  blockInfo->info.size = blockInfo->info.size*-1;
  add_free(blockInfo);
